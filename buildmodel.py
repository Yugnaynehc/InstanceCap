...
from inception_v3 import inception_v3_base, inception_v3_arg_scope
...
def process_image(mode, encoded_image, thread_id=0):
    ...
    return _process_image(encoded_image,
                          # If Traning, distort image; if None, crop central part; the
                          # size unchange.
...
def inception_v3(images,
    ...
    # Original      InceptionV3/Conv2d_1a_3x3/weights   ckpt don't have this
    # variable
    # Hao Dong                      InceptionV3/Conv2d_1a_3x3/weights:0
    # ckpt have this variable
...
def Build_Model(mode, net_image_embeddings, net_seq_embeddings, target_seqs, input_mask):
    ...
    if mode == 'inference':
        with tf.variable_scope("lstm", initializer=initializer) as lstm_scope:
            ...
            net_image_embeddings = tl.layers.ReshapeLayer(
                net_image_embeddings, shape=(1, 1, embedding_size))
        ...
        with tf.variable_scope("lstm", initializer=initializer) as lstm_scope:
            if mode == 'train':
                ...
                dropout = None
            net_image_embeddings = tl.layers.ReshapeLayer(
                net_image_embeddings, shape=(batch_size, 1, embedding_size))
            net_img_rnn = tl.layers.DynamicRNNLayer(net_image_embeddings,
                                                    cell_fn=tf.nn.rnn_cell.BasicLSTMCell,
                                                    n_hidden=num_lstm_units,
            ...
            # Then, uses the hidden state which contains image info as the
            # initial_state when feeding the sentence.
            lstm_scope.reuse_variables()
            tl.layers.set_name_reuse(True)
    ...
    network = tl.layers.DenseLayer(
        network, n_units=vocab_size, act=tf.identity, W_init=initializer, name="logits")  # TL
    ...
    if mode == "inference":
        ...
        batch_loss, losses, weights, _ = tl.cost.cross_entropy_seq_with_mask(
            logits, target_seqs, input_mask, return_details=True)    # TL
